...
/* ======================================================================== */
/* REPAIR — fix mutation-introduced runtime violations.                     */
/*                                                                          */
/* Mutation can drift numeric values into states the alien evaluator        */
/* rejects (e.g. (gate seq 2) → (gate seq 0) via a -2 delta) even though    */
/* the AST kind constraints are satisfied. This walk-and-clamp catches the  */
/* common cases that were spamming the Pd console:                          */
/*                                                                          */
/*   - positive-int slots (mod divisor, euclid steps, subdiv n, every n,    */
/*     gate n, speed division, cycle length, rep count): NUMBER children    */
/*     clamped to >= 1                                                      */
/*   - rand: arg count must be 1 or 3 (the AST metadata permits 1..3, but   */
/*     the runtime rejects 2); we promote 2 → 3 by appending a hi bound.    */
/*   - drunk: slots 3 (lo) and 4 (hi) swapped if lo > hi                    */
/*                                                                          */
/* Quantize-empty is a runtime-evaluation issue (a sub-expression that      */
/* evaluates to an empty sequence at run time); not addressed here because  */
/* it's not detectable from the AST without invoking the evaluator.         */
/* ======================================================================== */

/* Returns the minimum integer value allowed at child slot `slot` for `t`,  */
/* or 0 if there's no positive-int requirement at that slot.                */
static inline int ns_op_min_int_at(NodeType t, int slot) {
    switch (t) {
        case NODE_REP:    return slot == 1 ? 1 : 0;  /* repeat count */
        case NODE_MOD:    return slot == 1 ? 1 : 0;  /* divisor */
        case NODE_EUCLID: return slot == 1 ? 1 : 0;  /* steps */
        case NODE_SUBDIV: return slot == 1 ? 1 : 0;
        case NODE_EVERY:  return slot == 1 ? 1 : 0;
        case NODE_GATE:   return slot == 1 ? 1 : 0;
        case NODE_SPEED:  return slot == 1 ? 1 : 0;
        case NODE_CYCLE:  return slot == 1 ? 1 : 0;
        default: return 0;
    }
}

static inline void ns_ast_repair(ASTNode *n) {
    if (!n || ns_ast_is_leaf(n)) return;

    /* Recurse first — fix children before any structural fix-ups here. */
    for (int i = 0; i < n->data.op.child_count; i++) {
        ns_ast_repair(n->data.op.children[i]);
    }

    /* rand at child_count == 2 is a runtime error. Promote to 3 by adding
     * a hi bound above the existing slot[1] (treated as lo). If we can't
     * extend (alloc failure), drop slot[1] back to 1-arg form. */
    if (n->type == NODE_RAND && n->data.op.child_count == 2) {
        ASTNode *lo = n->data.op.children[1];
        int lo_v = (lo && lo->type == NODE_NUMBER) ? lo->data.number : 60;
        ASTNode *hi = ast_new_number(lo_v + 12);
        if (hi && !ast_add_child(n, hi)) { ast_free(hi); hi = NULL; }
        if (n->data.op.child_count == 2) {
            ast_free(n->data.op.children[1]);
            n->data.op.child_count = 1;
        }
    }

    /* Clamp positive-int slots. */
    for (int i = 0; i < n->data.op.child_count; i++) {
        ASTNode *c = n->data.op.children[i];
        if (!c || c->type != NODE_NUMBER) continue;
        int min_v = ns_op_min_int_at(n->type, i);
        if (min_v > 0 && c->data.number < min_v) c->data.number = min_v;
    }

    /* drunk: enforce slot 3 (lo) <= slot 4 (hi) by swap. */
    if (n->type == NODE_DRUNK && n->data.op.child_count >= 5) {
        ASTNode *lo = n->data.op.children[3];
        ASTNode *hi = n->data.op.children[4];
        if (lo && hi && lo->type == NODE_NUMBER && hi->type == NODE_NUMBER &&
            lo->data.number > hi->data.number) {
            int tmp = lo->data.number;
            lo->data.number = hi->data.number;
            hi->data.number = tmp;
        }
    }

    /* fold/wrap/clamp: enforce slot 1 (lo) < slot 2 (hi). fold and wrap
     * require strictly less-than (max > min); if equal, bump hi by 1. */
    if ((n->type == NODE_FOLD || n->type == NODE_WRAP || n->type == NODE_CLAMP)
        && n->data.op.child_count >= 3) {
        ASTNode *lo = n->data.op.children[1];
        ASTNode *hi = n->data.op.children[2];
        if (lo && hi && lo->type == NODE_NUMBER && hi->type == NODE_NUMBER) {
            if (lo->data.number > hi->data.number) {
                int tmp = lo->data.number;
                lo->data.number = hi->data.number;
                hi->data.number = tmp;
            }
            if (lo->data.number == hi->data.number &&
                (n->type == NODE_FOLD || n->type == NODE_WRAP)) {
                hi->data.number = lo->data.number + 1;
            }
        }
    }

    /* range: optional slot 2 (step) must be non-zero. */
    if (n->type == NODE_RANGE && n->data.op.child_count >= 3) {
        ASTNode *step = n->data.op.children[2];
        if (step && step->type == NODE_NUMBER && step->data.number == 0) {
            step->data.number = 1;
        }
    }

    /* scale: slot 1 (from_min) and slot 2 (from_max) must differ. */
    if (n->type == NODE_SCALE && n->data.op.child_count >= 5) {
        ASTNode *fmin = n->data.op.children[1];
        ASTNode *fmax = n->data.op.children[2];
        if (fmin && fmax && fmin->type == NODE_NUMBER && fmax->type == NODE_NUMBER &&
            fmin->data.number == fmax->data.number) {
            fmax->data.number = fmin->data.number + 1;
        }
    }
}
